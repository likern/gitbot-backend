...
class GitHub:
    def __init__(self, jwt_token: str, http_client: aiohttp.ClientSession = None, base_url: str = BASE_URL):
    ...
    def post(json: pydantic.BaseModel):
    ...
    async def post(self, url_path: str, obj_type=pydantic.BaseModel, **kwargs):

        if not issubclass(obj_type, pydantic.BaseModel):
            raise TypeError(
                f"{obj_type} should be subclass of pydantic.BaseModel")
    ...
    async def get(self, url_path: str):
        ...
        json_payload = model.dict(skip_defaults=True)
        url = f"{self._base_url}{url_path}"
        async with self.http_client.get(url, json=json_payload) as resp:
            print(f"Response status [{resp.status}]")
            json = await resp.json()
            return json
            print(f"Response body: [{json}]")
